...
class Zone(models.Model):
    """ Zone - A container of multiple sensors. Sensors are grouped by Zone in the UI and API.
    - By default, sort these by name.
    - API Key is automatically generated when we create new Zones.
      This is set in pre_save listener in zone_set_api_key in sensors.signals.
    """
...
class Sensor(models.Model):
    """ Sensor Device
     - Each sensor has a GUID. This is a unique identifer pulled from the
       device itself, it could be a MAC address, serial number, etc. It
       doesn't matter, it just needs to be unique.
     - Minimum and maximum value checks are optional, they won't be checked
       if they are left blank. This means each sensor can have 0, 1 or 2 checks.
     - If the sensor fails the check, everyone in the associated alert groups
    ...
    create_date = models.DateTimeField(
        "Sensor Create Date",
        auto_now_add=True,
        help_text=("Automatically set to the date the sensor is saved. ",
                   "Used for helping determine if the device is down."),
    )
    ...
    def check_value(self, value):
        ...
        TODO:
        - Some idiot could add an operator to only OPERATOR_CHOICES, resulting
          in a key error below. Validate for this.
        - Data model assumes we could have 0, 1, or 2 checks. Fine for this
          project. Not OK for other projects.
        """
