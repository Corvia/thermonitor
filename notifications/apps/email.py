...
class Email(BaseAlert):
    ...
    def down(self):
        ...
        msg = "Please check {0} at {1}, temperature monitoring is currently down."
        self.message = msg.format(self.sensor.name, self.sensor.zone.name)

    # Sensor temperature is now within threshold checks or has come back online.
    def recovered(self):
        self.subject = ("%s Temperature is OK - %s 'F" %
                        (self.sensor.name, self.sensor_data.value_f()))

        msg = "Temperature of {0} at {1} appears to have recovered, now reporting {2} 'F."
        self.message = str(msg).format(
            self.sensor.name,
            self.sensor.zone.name,
            self.sensor_data.value_f()
        )
    ...
    def send(self):
        """ Deliver a multi-part text/HTML email using the django template
        framework to render the parts of the email. In the future we may want to
        BCC contacts or send separate emails to each individual to provide
